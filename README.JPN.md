[简体中文](README.CHS.md) | [正體中文](README.CHT.md) | [English](README.md) | [日本語](README.JPN.md)

# Rulia - Windows用のモダンなマンガリーダー

新しい読書体験を提供するマンガリーダー。

Microsoft Storeから入手：

<a href="https://apps.microsoft.com/store/detail/9MVVLRZWRXX8?cid=github&launch=true&mode=mini">
  <img src="https://get.microsoft.com/images/ja-jp%20dark.svg" width="250" />
</a>

## 特徴

 - 一般的なファイル形式に対応：CBR, CBZ, ZIP, RAR, 7Z, PDF, EPUB, MOBI, AZW3
 - WebPや[AVIF](https://github.com/RuliaReader/Rulia/wiki/AVIF-Support)を含む様々な画像形式に対応
 - 3 種類の読み取りモード：ページめくりモード（フリップ）、無限モード（スクロール）、[ブックリーダーモード](https://github.com/RuliaReader/Rulia/issues/214)（実際の本のような）
 - 画像のモアレ（干渉縞）を除去
 - 画像の余白を自動でトリミング
 - [超解像技術](https://github.com/RuliaReader/Rulia/wiki/Super%E2%80%90resolution-for-manga-images)による低解像度画像の高画質化
 - 暗号化された圧縮ファイルの閲覧とパスワード記憶に対応
 - タッチ操作に最適化されたモダンなユーザーインターフェース
 - 複数のプロファイル作成によるマンガコレクションの管理
 - Surface PenとDialによるページめくりに対応
 - 自由にズーム：特定の画像または全体ビューを拡大 
 - [コマンドパレット](https://github.com/RuliaReader/Rulia/issues/239)
 - 機能拡張のための[プラグインシステム](https://github.com/RuliaReader/Rulia/wiki/Plugin-system)

## フィードバック

ご質問や提案がある場合は、Issueよりフィードバックをお願いします 😃

## ロードマップ

本リポジトリの[Issues](https://github.com/LancerComet/RuliaReader/issues)または[Projects](https://github.com/LancerComet/RuliaReader/projects)ページで進行中のタスクを確認できます。

## 変更履歴

変更履歴は[こちら](https://github.com/LancerComet/RuliaReader/blob/master/CHANGELOG.md)で確認できます。

## コミュニティ

 - [https://discord.gg/kewQHAQU4A](https://discord.gg/kewQHAQU4A)
 - QQグループ：623730051（主に中国語ユーザー向け）

## スクリーンショット

![rulia](/images/en-01.jpg)
![rulia](/images/en-02.jpg)
![rulia](/images/en-03.jpg)
![rulia](/images/en-04.jpg)

## 著作権

© 2023 LancerComet
