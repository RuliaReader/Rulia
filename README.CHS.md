[简体中文](README.CHS.md) | [正體中文](README.CHT.md) | [English](README.md) | [日本語](README.JPN.md)

# Rulia - 现代化设计的本地漫画阅读器

为您带来焕然一新的漫画阅读体验。

从 Microsoft Store 获取：

<a href="https://apps.microsoft.com/store/detail/9MVVLRZWRXX8?cid=github&launch=true&mode=mini">
  <img src="https://get.microsoft.com/images/zh-cn%20dark.svg" width="250" />
</a>

## 功能
 
 - 支持常见文件格式：CBR, CBZ, ZIP, RAR, 7Z, PDF, EPUB, MOBI, AZW3
 - 支持常见图片格式，包括 WebP 与 [AVIF](https://github.com/RuliaReader/Rulia/wiki/AVIF-Support)
 - 提供三种阅读模式：翻动模式（像相册）、无限模式（横向或纵向滚动）、[书本模式](https://github.com/RuliaReader/Rulia/issues/214)（仿真翻书）
 - 去除图片中的摩尔纹噪声
 - 自动裁剪图片边缘的空白区域
 - 利用[超分辨率技术](https://github.com/RuliaReader/Rulia/wiki/Super%E2%80%90resolution-for-manga-images)修复并放大低分辨率图片
 - 支持直接读取加密压缩包并记忆密码
 - 为触摸操作优化的现代化界面设计
 - 支持创建多个配置文件以管理不同的漫画收藏
 - 支持使用 Surface Pen 和 Dial 进行翻页
 - 自由缩放：放大特定图片或整体视图
 - [命令面板](https://github.com/RuliaReader/Rulia/issues/239) 
 - 支持[插件系统](https://github.com/RuliaReader/Rulia/wiki/Plugin-system)以扩展功能

## 反馈

如果您有任何问题或建议，欢迎提交 Issue 反馈 😃

## 开发计划 (Roadmap)

您可以在本项目的 [Issues](https://github.com/LancerComet/RuliaReader/issues) 或 [Projects](https://github.com/LancerComet/RuliaReader/projects) 页面查看当前的开发计划。

## 更新日志

你可以在[这里](https://github.com/LancerComet/RuliaReader/blob/master/CHANGELOG.md)阅读更新日志。

## 社群

 - [https://discord.gg/kewQHAQU4A](https://discord.gg/kewQHAQU4A)
 - QQ 群: 623730051（主要面向中文用户）

## 截图

![rulia](/images/chs-01.jpg)
![rulia](/images/chs-02.jpg)
![rulia](/images/chs-03.jpg)
![rulia](/images/chs-04.jpg)

## Copyright

© 2023 LancerComet
