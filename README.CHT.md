[简体中文](README.CHS.md) | [正體中文](README.CHT.md) | [English](README.md) | [日本語](README.JPN.md)

# Rulia - 現代化設計的本機漫畫閱讀器

為您打造與眾不同的漫畫閱讀體驗。

從 Microsoft Store 獲取：

<a href="https://apps.microsoft.com/store/detail/9MVVLRZWRXX8?cid=github&launch=true&mode=mini">
  <img src="https://get.microsoft.com/images/zh-tw%20dark.svg" width="250" />
</a>

## 功能

 - 支援常見檔案格式：CBR, CBZ, ZIP, RAR, 7Z, PDF, EPUB, MOBI, AZW3
 - 支援常見圖片檔案，包含 WebP 和 [AVIF](https://github.com/RuliaReader/Rulia/wiki/AVIF-Support)
 - 提供三種閱讀模式：翻動模式（類似相冊）、無限模式（橫向或縱向滾動）、[書本模式](https://github.com/RuliaReader/Rulia/issues/214)（仿真書籍）
 - 去除圖片中的摩爾紋噪點
 - 自動裁切圖片邊緣的空白區域
 - 利用[超解析度技術](https://github.com/RuliaReader/Rulia/wiki/Super%E2%80%90resolution-for-manga-images)修復並放大低畫質圖片
 - 支援直接讀取加密壓縮檔並記憶密碼
 - 專為觸控操作優化的現代化介面設計
 - 支援建立多個設定檔以管理不同的漫畫收藏
 - 支援使用 Surface Pen 和 Dial 進行翻頁
 - 自由縮放：放大特定圖片或整體視圖
 - [命令面板](https://github.com/RuliaReader/Rulia/issues/239) 
 - 支援[插件系統](https://github.com/RuliaReader/Rulia/wiki/Plugin-system)以擴充功能
 - [AI 翻譯](https://github.com/RuliaReader/Rulia/wiki/AI-Translation)

## 回饋

若您有任何疑問或建議，歡迎提交 Issue 回饋 😃

## 開發計畫 (Roadmap)

您可以在本專案的 [Issues](https://github.com/LancerComet/RuliaReader/issues) 或 [Projects](https://github.com/LancerComet/RuliaReader/projects) 頁面查看目前的開發計畫。

## 更新日誌

您可以在[這裡](https://github.com/LancerComet/RuliaReader/blob/master/CHANGELOG.md)閱讀更新日誌。

## 社群

 - [https://discord.gg/kewQHAQU4A](https://discord.gg/kewQHAQU4A)
 - QQ 群: 623730051（主要服務中文用戶）

## 截圖

![rulia](/images/chs-01.jpg)
![rulia](/images/chs-02.jpg)
![rulia](/images/chs-03.jpg)
![rulia](/images/chs-04.jpg)

## 版權

© 2023 LancerComet
